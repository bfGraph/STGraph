...
class SeastarGCNLayer(nn.Module):
    ...
    def forward(self, g, h, edge_weight=None):
        ...
        def nb_compute(v):
            ...
            h = sum(
                [
                    nb_edge.src[0].norm * nb_edge.src[0].h * nb_edge.weight
...
class SeastarTGCNCell(torch.nn.Module):
    ...
    def _calculate_reset_gate(self, g, X, edge_weight, H):
        R = torch.cat(
            (self.conv_r(g, X, edge_weight=edge_weight), H), axis=1
        )  # axis values need to be checked
        R = self.linear_r(R)
        R = torch.sigmoid(R)
        return R
    ...
    def _calculate_candidate_state(self, g, X, edge_weight, H, R):
        H_tilde = torch.cat(
            (self.conv_h(g, X, edge_weight=edge_weight), H * R), axis=1
        )  # axis values need to be checked
