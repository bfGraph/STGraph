...
CSR::CSR(std::vector<std::tuple<int, int, int>> edge_list, std::vector<float> edge_weight, int num_nodes, bool is_edge_reverse = false)
{
    ...
    std::vector<std::pair<int, int>> degree_id_pairs = std::vector<std::pair<int, int>>();
...
{
    cErr(cudaMemcpy(row_offset_device, row_offset.data(), row_offset.size() * sizeof(int), cudaMemcpyHostToDevice));
    cErr(cudaMemcpy(column_indices_device, column_indices.data(), column_indices.size() * sizeof(int), cudaMemcpyHostToDevice));
    cErr(cudaMemcpy(eids_device, eids.data(), eids.size() * sizeof(int), cudaMemcpyHostToDevice));
    cErr(cudaMemcpy(node_ids_device, node_ids.data(), node_ids.size() * sizeof(int), cudaMemcpyHostToDevice));
...
std::vector<int> get_array(std::uintptr_t ptr, int size)
{
    ...
    std::vector<int> vec(host_ptr,host_ptr+size);
    return vec;
}
