...
{
    uint64_t n = get_n();
    vector<tuple<uint32_t, uint32_t, uint32_t>> output(edge_count);

    cout << "Starting of Edges" << std::flush;
    ...
    cout << "Ending of Edges" << std::flush;
    return output;
}
...
std::tuple<std::size_t, std::size_t, std::size_t> PCSR::get_csr_ptrs(std::vector<int> eids)
{
    ...
    thrust::host_vector<int> column_indices(edge_count);
    thrust::host_vector<int> eids_host(edge_count);

    cout << "Starting th get pcsr on nodes " << nodes.size() << " and col_indices " << edges.items.size() << std::endl
         << std::flush;
    ...
    for (int i = 0; i < column_indices_size; ++i)
    {
        ...
        {
            column_indices[edge_iter] = edges.items[i].dest;
            ++edge_iter;
        }
    }
    ...
    cout << "Edge Count: " << edge_count << "  Edge iter: " << edge_iter << std::endl
         << std::flush;
    ...
    if (eids.size() == 0)
    {
        thrust::sequence(eids_host.begin(), eids_host.end());
    }
    else
    {
        // copying eids
        std::cout << "edge count: " << edge_count << ", eid.size(): " << eids.size() << std::endl
                  << std::flush;
        thrust::copy(eids.begin(), eids.end(), eids_host.begin());
    }
    std::cout << "End Processing\n"
              << std::flush;

    eids_device = eids_host;
